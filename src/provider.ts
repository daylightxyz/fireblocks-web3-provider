...
import {
  DEBUG_NAMESPACE_ENHANCED_ERROR_HANDLING,
  DEBUG_NAMESPACE_REQUESTS_AND_RESPONSES,
  DEBUG_NAMESPACE_TX_STATUS_CHANGES,
  FINAL_SUCCESSFUL_TRANSACTION_STATES,
...
export class FireblocksWeb3Provider extends HttpProvider {
  ...
  constructor(config: FireblocksProviderConfig) {
    ...
    if (
      config.enhancedErrorHandling ||
      config.enhancedErrorHandling == undefined
    ) {
    ...
    if (
      config.rpcUrl &&
      config.rpcUrl.includes("@") &&
      config.rpcUrl.includes(":")
    ) {
      const [creds, url] = config.rpcUrl
        .replace("https://", "")
        .replace("http://", "")
        .split("@");
      config.rpcUrl = `${
        config.rpcUrl.startsWith("https") ? "https://" : "http://"
      }${url}`;
      headers.push({
        name: "Authorization",
  ...
  private parsePrivateKey(privateKey: string): string {
    if (!privateKey) {
      throw Error(
        `privateKey is required in the fireblocks-web3-provider config`
      );
    }
  ...
  private async populateGaslessGasTankAddress(): Promise<void> {
    ...
    const depositAddresses =
      await this.fireblocksApiClient.getPaginatedAddresses(
        this.gaslessGasTankVaultId!.toString(),
        this.assetId!
      );
    if (depositAddresses.addresses.length === 0) {
      throw Error(
        `Gasless gas tank vault not found (vault id: ${this.gaslessGasTankVaultId})`
      );
    }
    this.gaslessGasTankVaultAddress = depositAddresses.addresses[0].address;
    this.accounts[this.gaslessGasTankVaultId!] =
      this.gaslessGasTankVaultAddress;
  }

  private parseVaultAccountIds(
    vaultAccountIds: number | number[] | string | string[] | undefined
  ): number[] | undefined {
  ...
  private async getVaultAccounts(): Promise<number[]> {
    ...
    return (
      await this.fireblocksApiClient.getVaultAccountsWithPageInfo({
  ...
  private async populateAssetAndChainId() {
    const chainId = (
      await util.promisify<any, any>(super.send).bind(this)(
  ...
  private async populateAccounts() {
    ...
    for (const vaultAccountId of this.vaultAccountIds) {
      ...
      if (
        this.config.vaultAccountIds &&
        depositAddresses.addresses.length == 0
      ) {
        throw this.createError({
          message: `No ${this.assetId} asset wallet found for vault account with id ${vaultAccountId}`,
        });
      }
  ...
  private createFireblocksError(e: any) {
    ...
    message = e?.response?.data?.code
      ? `${message} (Error code: ${e.response.data.code})`
      : message;
    message = e?.response?.headers?.["x-request-id"]
      ? `${message} (Request ID: ${e.response.headers["x-request-id"]})`
      : message;

    return this.createError({ message, code });
  }

  private async getWhitelistedWallets(
    walletsPromise: Promise<any>,
    type: PeerType,
    assetId: string
  ) {
  ...
  private async populateWhitelisted() {
    ...
    const [externalWallets, internalWallets, contractWallets] =
      await Promise.all([
        this.getWhitelistedWallets(
          this.fireblocksApiClient.getExternalWallets(),
          PeerType.EXTERNAL_WALLET,
          this.assetId!
        ),
        this.getWhitelistedWallets(
          this.fireblocksApiClient.getInternalWallets(),
          PeerType.INTERNAL_WALLET,
          this.assetId!
        ),
        this.getWhitelistedWallets(
          this.fireblocksApiClient.getContractWallets(),
          PeerType.EXTERNAL_WALLET,
          this.assetId!
        ),
      ]);
    ...
    (async () => {
      ...
      try {
        logRequestsAndResponses(
          `Request #${requestNumber}: method=${
            payload.method
          } params=${JSON.stringify(payload.params, undefined, 4)}`
        );
        ...
        switch (payload.method) {
          ...
          case "eth_accounts":
            ...
            result = Object.values(this.accounts).filter(
              (addr: any) =>
                addr.toLowerCase() !=
                this.gaslessGasTankVaultAddress.toLowerCase()
            );
            break;
          ...
          case "eth_sendTransaction":
            ...
            try {
              if (this.isZora()) {
                ...
                this.gaslessGasTankVaultId != undefined &&
              ...
              logEnhancedErrorHandling(
                `Simulate the failed transaction on Tenderly: ${this.createTenderlySimulationLink(
                  payload.params[0]
                )}`
              );
              throw error;
          ...
          default:
            const jsonRpcResponse = await util
      ...
      if (error) {
        ...
        logRequestsAndResponses(
          `Response #${requestNumber}: ${JSON.stringify(result, undefined, 4)}`
        );
      }
  ...
  public async request(args: RequestArguments): Promise<any> {
    return (
      await util.promisify(this.send).bind(this)(
        formatJsonRpcRequest(args.method, args.params)
      )
    ).result;
  }
  ...
  private getDestination(address: string): DestinationTransferPeerPath {
    if (this.oneTimeAddressesEnabled) {
      ...
      if (!address || address == "0x0") {
        throw this.createError({
          message:
            "Contract deployment is currently not available without enabling one-time addresses",
        });
      }
      ...
      if (!whitelistedDestination) {
        throw this.createError({
          message: `Address ${address} is not whitelisted. Whitelisted addresses: ${JSON.stringify(
            this.whitelisted,
            undefined,
            4
          )}`,
...
${
  !this.config.vaultAccountIds
    ? "vaultAccountIds was not provided in the configuration. When that happens, the provider loads the first 20 vault accounts found. It is advised to explicitly pass the required vaultAccountIds in the configuration to the provider."
    : `vaultAccountIds provided in the configuration: ${this.vaultAccountIds!.join(
        ", "
      )}`
}.
Available addresses: ${Object.values(this.accounts).join(", ")}.`,
  ...
  private async createGaslessTransaction(transaction: any) {
    ...
    if (
      transaction.chainId &&
      Number(transaction.chainId) != Number(this.chainId)
    ) {
      throw new Error(
        `Chain ID of the transaction (${transaction.chainId}) does not match the chain ID of the FireblocksWeb3Provider (${this.chainId})`
      );
    }
    ...
    const NativeMetaTransactionContract =
      NativeMetaTransaction__factory.connect(
        to,
        ethersProvider.getSigner(this.gaslessGasTankVaultAddress)
      );
    ...
    const signature = ethers.utils.splitSignature(
      await ethersProvider.getSigner(from)._signTypedData(domain, types, req)
    );

    const relayedTx =
      await NativeMetaTransactionContract.executeMetaTransaction(
        from,
        data,
        signature.r,
        signature.s,
  ...
  private async createContractCall(transaction: any) {
    ...
    if (
      ...
      throw this.createError({
        message: `Chain ID of the transaction (${transaction.chainId}) does not match the chain ID of the FireblocksWeb3Provider (${this.chainId})`,
      });
    }

    if (!transaction.from) {
      throw this.createError({
        message: `Transaction sent with no "from" field`,
      });
    }

    const vaultAccountId = this.getVaultAccountIdAndValidateExistence(
      transaction.from,
      `Transaction sent from an unsupported address: `
    );
    ...
    const isEip1559Fees: boolean =
      Boolean(maxFee) && Boolean(maxPriorityFeePerGas) && Boolean(gas);
    const isLegacyFees: boolean =
      Boolean(gasPrice) && Boolean(gas) && !isEip1559Fees;
    ...
    const transactionArguments: TransactionArguments = {
      ...
      gasLimit:
        isEip1559Fees || isLegacyFees ? Number(gas).toString(10) : undefined,
      ...
      externalTxId: !this.externalTxId
        ? undefined
        : typeof this.externalTxId == "function"
        ? this.externalTxId()
        : this.externalTxId,
  ...
  private async createRawSignedTransaction(transaction: {
    from: string;
    to: string;
    nonce?: string;
    gas?: string;
    ...
    if (!transaction.from) {
      throw this.createError({
        message: `Can't raw-sign transaction without "from" field`,
      });
    }
    ...
    const unsignedTransaction: ethers.UnsignedTransaction = {
      type: typeof transaction.maxFeePerGas !== undefined ? 2 : undefined,
    ...
    const transactionArguments: TransactionArguments = {
      ...
      extraParameters: {
        rawMessageData: {
          ...
          messages: [
            {
              derivationPath: this.getDerivationPath(vaultAccountId),
              content: keccak256(serializedTransaction).substring(2),
              preHash: {
                content: serializedTransaction.substring(2),
    ...
    const res = await this.createTransaction(transactionArguments);
    const sig = res.signedMessages?.[0]?.signature;

    if (!sig) {
      throw this.createError({
        message: `[txid ${res.id}] Failed to get back signature`,
      });
    }
  ...
  private async createPersonalSign(
    ...
    const vaultAccountId = this.getVaultAccountIdAndValidateExistence(
      address,
      `Signature request from an unsupported address: `
    );
  ...
  getDerivationPath(vaultAccountId: number): number[] {
    return [44, 60, vaultAccountId, 0, 0];
  }

  private async createRawSign(
    address: string,
    content: any,
    type: RawMessageType
  ): Promise<string> {
      ...
      extraParameters: {
        rawMessageData: {
          ...
          messages: [
            {
              derivationPath: this.getDerivationPath(vaultAccountId),
              content: keccak256(finalContent).substring(2),
              preHash: {
                content: finalContent.substring(2),
                hashAlgorithm: "KECCAK256",
              },
            },
          ],
  ...
  private async createTransaction(
    ...
    const { id } = await this.fireblocksApiClient.createTransaction(
      transactionArguments
    );
        ...
        if (currentStatus != txInfo.status) {
          logTransactionStatusChange(
            `Fireblocks transaction ${
              txInfo.id
            } changed status from ${currentStatus} to ${txInfo.status} ${
    ...
    if (!FINAL_SUCCESSFUL_TRANSACTION_STATES.includes(currentStatus)) {
      throw this.createError({
        message: `Fireblocks transaction ${
          txInfo!.id || ""
        } was not completed successfully. Final Status: ${currentStatus} ${
          txInfo!?.subStatus ? `(${txInfo!?.subStatus})` : ""
        }`,
      });
    }
  ...
  private getVaultAccountId(address: string): number {
    return parseInt(
      Object.entries(this.accounts).find(
        ([id, addr]) => addr.toLowerCase() === address.toLowerCase()
      )?.[0] || ""
    );
  }
